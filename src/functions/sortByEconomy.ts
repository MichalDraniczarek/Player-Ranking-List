...
function bubbleSortV1playerTotalPoints(playerOb, tempPlayerCell){
    ...
    for(let j = 0; j < playerOb.length; j++){
        for(let i = 0; i < playerOb.length - 1; i++){
            if(playerOb[i].playerEconomyPoints > playerOb[i + 1].playerEconomyPoints){
...
function selectionSortMethod(playerOb){
    ...
    for(let j = 0; j < playerOb.length - 1; j++){
        ...
        for(let i = j + 1; i < playerOb.length; i++){
            if(playerOb[i].playerEconomyPoints < playerOb[index].playerEconomyPoints){
                index = i;
            }
            operationCounter++;
        }
...
function insertionSortMethod(playerOb){
    ...
    let operationCounter:number = 0;
        ...
        {
          let tempSortingElement = playerOb[j].playerEconomyPoints;
          let tempObjectStore = playerOb[j];
          let i = j + 1;
          while((i < playerOb.length) && (tempSortingElement > playerOb[i].playerEconomyPoints))
          {
...
function quickSortFnMethod(playerOb){
    ...
    function quickSort(lewy, prawy){
        ...
        piwot = playerOb[i].playerEconomyPoints;
        tempObI  = playerOb[i];
        playerOb[i] = playerOb[prawy];
        for(j = i = lewy; i < prawy; i++)
        if(playerOb[i].playerEconomyPoints < piwot)
        {
            x = playerOb[i]; 
...
export function jsSortingFunctionForplayerEconomy(playerOb,selectSortingMethodHTML,rankingSortingState,valueOfSortingCalculations){
    ...
    if(rankingSortingState[1] == false){
    ...
    rankingSortingState[1] = true;
    }else{
        playerOb.reverse();

        restartRankingSetStartingPositionToZero();
        rankingSortingState[1] = false;
    }
